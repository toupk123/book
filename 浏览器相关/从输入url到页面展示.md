...
在http协议中，整个传输过程总共分为了五层。这五层分为

1. 应用层，也就是客户端或者服务端，其中应用层也包含表示层和会话层
2. 网络层，在这一层就是握手的地方，tcp协议和UDP协议
3. 传输层，IP协议
4. 链路层，以太网
5. 物理层，网卡之类的。

用户在输入一段网址时，在传输过程中实际上是使用的ip地址，但是ip地址使用起来不习惯，所以出现URL。

1. 浏览器首先会在浏览器本地查找有没有DNS缓存，如果有的话直接获取到ip
2. 如果没有获取到会去本机host文件中查找有没有ip
3. 如果2没有获取到的话，再去dns服务器中获取

### TCP协议以及挥手

TCP的目的是保证数据完整并且可靠的数据流传输,在传输开始前，都要进行事先开启一个tcp连接。

#### 三次握手

1. 客户端发送一个SYN为1段指明客户打算连接服务端的端口，以及序列号。
2.  服务器发回包含服务器初始序列号的SYN作为应答。同时将确认序号设置为客户的ack加1回应客户。
3. 客户必须将明确序号设置为服务器的seq加1，对服务器的SYN报文进行确认

![avatar](../imgea/WeChatdb7693ab1a10e891aac94bd72e4d3876.png)

#### 为什么不是两次握手

因为服务端无法保证客户接收到了回应，这样的话服务端就会继续向客户端发送请求。

#### 为什么不是四次握手

三次可以达到的事情，不需要第四次，这样会浪费资源



#### 四次挥手

![avatar](../imgea/D1EB1AFA-451D-4FDB-A6A6-11250EA1A160.png)

1. 由发起方发送一个FIN，发送完成之后，就变为FIN-WAIT-1，这个时候发起放就只能被动接受服务端发送的字段
2. 服务端接受后，向客户端发送ACK进行确认
3. 客户端接受到服务端发过来的确认信息时，自身就进入FIN-WAIT-2
4.  服务端向客户端发送`FIN`，自己进入`LAST-ACK`状态，

5. 客户端收到服务端发来的`FIN`后，自己变成了`TIME-WAIT`状态，然后发送 ACK 给服务端。

这个时候客户端需等待2个MSL(报文生成最大时间)那么表示ACK成功到达，挥手结束。



#### 为什么要等待2MSL的意义

确认服务端发送过来的资源全部发送完毕，如果不等待的话，当服务端还有很多数据包要给客户端的话，还在路上。若客户端现在端口被占用了，那么客户端就没法接受到。
...
2. 渲染进程（render进程），则是浏览器中一个页面的进程，含有：
...
cookie本身并不是为浏览器本地储存，而是为了记录这http的状态。cookie是非常不安全的。
